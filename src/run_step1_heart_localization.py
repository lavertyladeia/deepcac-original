...
# If CPU-only inference is requested, disable GPU before TensorFlow is imported
# (avoids "Blas SGEMM launch failed" on some GPU/driver/cuDNN setups)
if yaml_conf.get("processing", {}).get("use_cpu_inference", False):
...
num_gpus = yaml_conf["model"].get("num_gpus", 1)  # Default to 1 if not specified
